...
from .models import (
    ...
    Conversation, Message, NegotiationSettings,HeroSlide
)
...
class ProductVariationInline(admin.TabularInline):
    ...
    extra = 1
    fields = ('type', 'value', 'price_modifier', 'stock_variation')
...
class ProductAdmin(admin.ModelAdmin):
    list_display = ('name', 'shop', 'price', 'category', 'subcategory', 'stock', 'date_added')
    list_filter = ('category', 'subcategory', 'shop', 'date_added')
    search_fields = ('name', 'description', 'shop__merchant__first_name')
    list_editable = ('price', 'stock')
    prepopulated_fields = {}  # Vous pourriez ajouter un slug si nécessaire
...
# Inline pour CartItem (CORRIGÉ : lié à Cart, pas à Order)
...
class CartAdmin(admin.ModelAdmin):
    ...
    inlines = [CartItemInline]  # Ajout de l'inline ici
    
    def item_count(self, obj):
        return obj.items.count()
    item_count.short_description = 'Nombre d\'articles'

# Inline pour OrderItem (Nouveau inline pour Order)
...
class OrderAdmin(admin.ModelAdmin):
    ...
    inlines = [OrderItemInline]  # Utilisation du bon inline
